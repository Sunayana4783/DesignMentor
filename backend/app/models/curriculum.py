...
class Topic(Base):
    ...
    phase: Mapped[Phase] = mapped_column(Enum(Phase, native_enum=False), nullable=False)
...
class Concept(Base):
    ...
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    slug: Mapped[str] = mapped_column(String(200), unique=True, nullable=False, index=True)
    category: Mapped[ConceptCategory] = mapped_column(Enum(ConceptCategory, native_enum=False), nullable=False)
    difficulty: Mapped[DifficultyLevel] = mapped_column(Enum(DifficultyLevel, native_enum=False), nullable=False)
